//! OSU MPI Bandwidth Persistent Test (v7.5.2)
//!
//! Measures point-to-point message bandwidth using persistent requests
//! with a windowed send/recv pattern. Persistent requests are created
//! once per message size with MPI_Send_init/MPI_Recv_init, then
//! replayed each iteration with MPI_Startall/MPI_Waitall.
//!
//! Protocol (matches C reference exactly):
//! - Rank 0: posts `window_size` persistent sends -> waits for all -> receives 1-byte ACK
//! - Rank 1: posts `window_size` persistent recvs -> waits for all -> sends 1-byte ACK
//!
//! # Usage
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_bw_persistent
//! ```
...
/// Run the persistent bandwidth benchmark.
...
/// Perform a single persistent bandwidth iteration.
///
/// Rank 0: sends window_size messages, waits, receives ACK.
/// Rank 1: receives window_size messages, waits, sends ACK.
///
/// The persistent pattern: request params are created once and reused
/// across all iterations (amortized setup cost).
