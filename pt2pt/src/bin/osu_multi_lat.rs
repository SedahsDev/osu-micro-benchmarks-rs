//! OSU MPI Multi-message Latency Test (v7.5.2)
//!
//! Measures point-to-point message latency using UCX tag matching.
//! Sends multiple messages in a burst before receiving, measuring
//! round-trip latency for each message size.
//!
//! Protocol (matches C reference):
//! - Rank 0: sends message → receives message (ping-pong, one at a time)
//! - Rank 1: receives message → sends message (ping-pong, one at a time)
//!
//! This is essentially the same as osu_latency but with the "multi" naming
//! convention from the C reference, which supports multiple process pairs.
//! For our 2-process UCX implementation, we run a standard ping-pong.
//!
//! # Usage
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_multi_lat
//! ```
...
/// Run the multi-message latency benchmark using UCX tag matching.
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Multi-message Latency", BenchmarkType::Latency);
