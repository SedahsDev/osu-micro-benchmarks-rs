//! OSU MPI Bi-Directional Bandwidth Persistent Test (v7.5.2)
//!
//! Measures bidirectional point-to-point bandwidth using persistent requests.
...
//! - Rank 0: posts window of recv(from 1, tag=10) -> posts window of send(to 1, tag=100) -> waits all sends -> waits all receives
//! - Rank 1: posts window of recv(from 0, tag=100) -> posts window of send(to 0, tag=10) -> waits all receives -> waits all sends
//!
...
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_bibw_persistent
//! ```
...
/// UCX tag for rank 0 -> rank 1 data messages (matches C tag 100).
const TAG_0_TO_1: u64 = 100;
/// UCX tag for rank 1 -> rank 0 data messages (matches C tag 10).
const TAG_1_TO_0: u64 = 10;

/// Run the persistent bidirectional bandwidth benchmark.
...
/// Single persistent bidirectional bandwidth iteration.
///
/// Both ranks post receives first, then sends, then wait for all.
/// Matches the C reference pattern exactly:
