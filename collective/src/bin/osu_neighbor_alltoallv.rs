//! OSU MPI Neighborhood Alltoallv Latency Test
//!
//! Measures neighbor alltoallv latency using UCX tag-matching fallback.
//! Ring topology: each rank communicates with (rank-1)%size and (rank+1)%size.
//!
//! # Usage
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_neighbor_alltoallv
//! ```
...
/// Run the neighbor alltoallv latency benchmark.
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    // Allocate buffers — sendbuf and recvbuf each need 2 * max_message_size (2 neighbors)
    let max_buf_size = args.max_message_size;
    let mut sendbuf = vec![0u8; max_buf_size * 2];
    let mut recvbuf = vec![0u8; max_buf_size * 2];

    // For neighbor alltoallv with 2 neighbors, use equal counts/displacements
    let num_neighbors = 2;
    let mut send_counts = vec![0usize; num_neighbors];
    let mut send_displs = vec![0usize; num_neighbors];
    ...
    for msg_size in message_sizes(args) {
        ...
        // Initialize send buffer: each neighbor's chunk = rank + 1 + neighbor_idx
        for neighbor_idx in 0..num_neighbors {
            for i in 0..msg_size {
                sendbuf[neighbor_idx * msg_size + i] = (rank + 1 + neighbor_idx) as u8;
            }
        }

        // Set up counts and displacements for 2 neighbors
        ...
        let send_slice = &sendbuf[..msg_size * 2];
        let recv_slice = &mut recvbuf[..msg_size * 2];

        // Warmup iterations
        for _ in 0..skip {
            ctx.neighbor_alltoallv(send_slice, recv_slice, &send_counts, &send_displs, &recv_counts, &recv_displs);
            ctx.barrier();
        }
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Neighbor Alltoallv Latency", BenchmarkType::CollectiveLatency);
