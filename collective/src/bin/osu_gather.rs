//! OSU MPI Gather Latency Test (v7.5.2)
//!
//! Measures gather latency using UCX tag-matching fallback.
//!
//! Requires at least 2 processes. Each rank sends `msg_size` bytes to root;
//! root receives `msg_size * numprocs` total.
...
/// Run the gather latency benchmark.
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    for msg_size in message_sizes(args) {
        ...
        // Warmup iterations
        for _ in 0..skip {
            ctx.gather(send_slice, recv_slice, msg_size, 0);
            ctx.barrier();
        }
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Gather Latency", BenchmarkType::CollectiveLatency);
