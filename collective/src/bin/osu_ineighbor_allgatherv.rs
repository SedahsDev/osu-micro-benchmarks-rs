//! OSU MPI Non-blocking Ineighbor_allgatherv Latency Test (v7.5.2)
//!
//! Measures non-blocking neighbor allgatherv latency using a ring topology.
...
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    loop {
        ...
        for i in 0..(iterations + skip) {
            ...
            let mut request = ctx.ineighbor_allgatherv(&sendbuf, &mut recvbuf, &counts, &displs);
...
fn main() {
    ...
    if ctx.rank() == 0 {
        ...
        output::print_header(
            &mut out,
            "Non-blocking Ineighbor_allgatherv",
