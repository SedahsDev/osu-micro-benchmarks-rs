//! OSU MPI Non-blocking Reduce_scatter Latency Test (v7.5.2)
//!
//! Measures non-blocking collective reduce_scatter latency using UCX tag-matching
//! reduce_scatter fallback. Reports Overlap, CPU, Communication, Wait, and Init
//! times in microseconds.
//!
//! Requires at least 2 processes. Repeatedly calls ireduce_scatter + wait and
//! reports timing breakdown.
//!
//! # Usage
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_ireduce_scatter
//! ```
...
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    // Fixed message size for now (equal recvcounts)
    let msg_size = 0;
    let sendbuf = vec![0u8; msg_size * size];
    let mut recvbuf = vec![0u8; msg_size];
    let recvcounts = vec![msg_size; size];
    ...
    for i in 0..(iterations + skip) {
        ...
        let mut request: OsURequest = ctx.ireduce_scatter(&sendbuf, &mut recvbuf, &recvcounts);
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Non-blocking Reduce_scatter", BenchmarkType::NonBlockingCollective);
    }
