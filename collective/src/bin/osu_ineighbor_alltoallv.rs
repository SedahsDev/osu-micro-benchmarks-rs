//! OSU MPI Non-blocking Ineighbor_alltoallv Latency Test (v7.5.2)
//!
//! Measures non-blocking neighbor alltoallv latency using a ring topology.
...
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    loop {
        ...
        for i in 0..(iterations + skip) {
            ...
            let mut request = ctx.ineighbor_alltoallv(
                &sendbuf,
                &mut recvbuf,
                &send_counts,
                &recv_counts,
