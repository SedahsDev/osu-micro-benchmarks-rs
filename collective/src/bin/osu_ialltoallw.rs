//! OSU MPI Non-blocking Alltoallw Latency Test (v7.5.2)
//!
//! Measures non-blocking collective alltoallw latency using UCX tag-matching
//! alltoallw fallback. Reports Overlap, CPU, Communication, Wait, and Init
//! times in microseconds.
//!
//! Requires at least 2 processes. Repeatedly calls ialltoallw + wait and reports
//! timing breakdown.
//!
//! # Usage
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_ialltoallw
//! ```
...
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    // Fixed message size for now (equal counts/displacements)
    let msg_size = 0;
    let sendbuf = vec![0u8; msg_size * size];
    let mut recvbuf = vec![0u8; msg_size * size];
    let send_counts = vec![msg_size; size];
    let recv_counts = vec![msg_size; size];
    let send_displs: Vec<usize> = (0..size).map(|i| i * msg_size).collect();
    let recv_displs: Vec<usize> = (0..size).map(|i| i * msg_size).collect();
    ...
    for i in 0..(iterations + skip) {
        ...
        let mut request: OsURequest = ctx.ialltoallw(
            &sendbuf,
            &mut recvbuf,
            &send_counts,
            &recv_counts,
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Non-blocking Alltoallw", BenchmarkType::NonBlockingCollective);
    }
