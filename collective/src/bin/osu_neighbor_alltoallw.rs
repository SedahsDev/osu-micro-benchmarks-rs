//! OSU MPI Neighborhood Alltoallw Latency Test
//!
//! Measures neighbor alltoallw latency using UCX tag-matching fallback.
...
/// Run the neighbor alltoallw latency benchmark.
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    // For neighbor alltoallw with 2 neighbors, use equal counts/displacements
    let num_neighbors = 2;
    ...
    for msg_size in message_sizes(args) {
        ...
        // Warmup iterations
        for _ in 0..skip {
            ctx.neighbor_alltoallw(send_slice, recv_slice, &send_counts, &send_displs, &recv_counts, &recv_displs);
            ctx.barrier();
        }
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Neighbor Alltoallw Latency", BenchmarkType::CollectiveLatency);
