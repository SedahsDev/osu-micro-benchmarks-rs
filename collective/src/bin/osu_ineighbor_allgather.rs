//! OSU MPI Non-blocking Ineighbor_allgather Latency Test (v7.5.2)
//!
//! Measures non-blocking neighbor allgather latency using a ring topology
//! where each rank communicates only with its immediate neighbors.
...
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    loop {
        ...
        for i in 0..(iterations + skip) {
            ...
            let mut request = ctx.ineighbor_allgather(&sendbuf, &mut recvbuf, msg_size);
        ...
        if rank == 0 {
            ...
            output::print_nbc_row(&mut out, msg_size, overlap, cpu_avg, comm_avg, wait_avg, init_avg);
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Non-blocking Ineighbor_allgather", BenchmarkType::NonBlockingCollective);
    }
