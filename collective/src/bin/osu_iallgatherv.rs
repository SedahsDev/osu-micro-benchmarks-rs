//! OSU MPI Non-blocking Allgatherv Latency Test (v7.5.2)
//!
//! Measures non-blocking collective allgatherv latency using UCX tag-matching
//! allgatherv fallback. Reports Overlap, CPU, Communication, Wait, and Init
//! times in microseconds.
//!
//! Requires at least 2 processes. Repeatedly calls iallgatherv + wait and reports
//! timing breakdown.
//!
//! # Usage
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_iallgatherv
//! ```
...
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    // Fixed message size for now (equal counts)
    let msg_size = 0;
    let sendbuf = vec![0u8; msg_size];
    let mut recvbuf = vec![0u8; msg_size * size];
    let counts = vec![msg_size; size];
    let displs: Vec<usize> = (0..size).map(|i| i * msg_size).collect();
    ...
    for i in 0..(iterations + skip) {
        ...
        let mut request: OsURequest = ctx.iallgatherv(&sendbuf, &mut recvbuf, &counts, &displs);
        let init_time = init_start.elapsed_us();

        // CPU/compute time: do dummy work while collective is in flight
        let comp_start = Wtime::new();
        // No real dummy compute for iallgatherv — just measure the overlap
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Non-blocking Allgatherv", BenchmarkType::NonBlockingCollective);
    }
