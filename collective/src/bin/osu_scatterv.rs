//! OSU MPI Scatterv Latency Test (v7.5.2)
//!
//! Measures scatterv latency using UCX tag-matching fallback.
//!
//! Requires at least 2 processes. Root sends `msg_size * numprocs` bytes;
//! each rank receives `msg_size` bytes with variable counts/displacements.
...
/// Run the scatterv latency benchmark.
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    for msg_size in message_sizes(args) {
        ...
        // Warmup iterations
        for _ in 0..skip {
            ctx.scatterv(send_slice, recv_slice, &recvcounts, &rdispls, 0);
            ctx.barrier();
        }
