//! OSU MPI Alltoall Latency Test (v7.5.2)
//!
//! Measures alltoall latency using UCX tag-matching fallback.
//!
//! Requires at least 2 processes. Runs alltoall with various message sizes,
//! reporting latency in microseconds.
//!
//! # Usage
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_alltoall
//! ```
...
/// Run the alltoall latency benchmark.
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    for msg_size in message_sizes(args) {
        ...
        // Warmup iterations
        for _ in 0..skip {
            ctx.alltoall(send_slice, recv_slice, msg_size);
            ctx.barrier();
        }
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Alltoall Latency", BenchmarkType::CollectiveLatency);
