//! OSU MPI Non-blocking Gatherv Latency Test (v7.5.2)
//!
//! Measures non-blocking collective gatherv latency using UCX tag-matching
//! gatherv fallback. Reports Overlap, CPU, Communication, Wait, and Init
//! times in microseconds.
//!
//! Requires at least 2 processes. Repeatedly calls igatherv + wait and reports
//! timing breakdown.
//!
//! # Usage
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_igatherv
//! ```
...
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    for i in 0..(iterations + skip) {
        ...
        let mut request: OsURequest = ctx.igatherv(&sendbuf, &mut recvbuf, 0, msg_size, msg_size);
        let init_time = init_start.elapsed_us();

        // CPU/compute time: do dummy work while collective is in flight
        let comp_start = Wtime::new();
        // No real dummy compute for igatherv — just measure the overlap
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Non-blocking Gatherv", BenchmarkType::NonBlockingCollective);
    }
