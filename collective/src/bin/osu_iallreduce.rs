//! OSU MPI Non-blocking Allreduce Latency Test (v7.5.2)
//!
//! Measures non-blocking collective allreduce latency using UCX tag-matching
//! allreduce fallback. Reports Overlap, CPU, Communication, Wait, and Init
//! times in microseconds.
//!
//! Requires at least 2 processes. Repeatedly calls iallreduce + wait and reports
//! timing breakdown.
//!
//! # Usage
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_iallreduce
//! ```
...
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    // Fixed message size for now (single u64 value)
    let msg_size = 0;
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Non-blocking Allreduce", BenchmarkType::NonBlockingCollective);
    }
