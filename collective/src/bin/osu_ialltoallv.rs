//! OSU MPI Non-blocking Alltoallv Latency Test (v7.5.2)
//!
//! Measures non-blocking collective alltoallv latency using UCX tag-matching
//! alltoallv fallback. Reports Overlap, CPU, Communication, Wait, and Init
//! times in microseconds.
//!
//! Requires at least 2 processes. Repeatedly calls ialltoallv + wait and reports
//! timing breakdown.
//!
//! # Usage
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_ialltoallv
//! ```
...
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    for i in 0..(iterations + skip) {
        ...
        let mut request: OsURequest = ctx.ialltoallv(
            &sendbuf,
            &mut recvbuf,
            &send_counts,
            &recv_counts,
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Non-blocking Alltoallv", BenchmarkType::NonBlockingCollective);
    }
