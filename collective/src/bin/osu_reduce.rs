//! OSU MPI Reduce Latency Test (v7.5.2)
//!
//! Measures reduce latency using UCX tag-matching fallback.
//!
//! Requires at least 2 processes. Runs reduce with various message sizes,
//! reporting latency in microseconds.
//!
//! # Usage
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_reduce
//! ```
...
/// Run the reduce latency benchmark.
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    for msg_size in message_sizes(args) {
        ...
        // Warmup iterations
        for _ in 0..skip {
            ctx.reduce(send_slice, recv_slice, 0);
            ctx.barrier();
        }
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Reduce Latency", BenchmarkType::CollectiveLatency);
