//! OSU MPI Non-blocking Reduce_scatter_block Latency Test (v7.5.2)
//!
//! Measures non-blocking collective reduce_scatter_block latency using UCX tag-matching
//! reduce_scatter_block fallback. Reports Overlap, CPU, Communication, Wait, and Init
//! times in microseconds.
//!
//! Requires at least 2 processes. Repeatedly calls ireduce_scatter_block + wait and
//! reports timing breakdown.
//!
//! # Usage
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_ireduce_scatter_block
//! ```
...
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    for i in 0..(iterations + skip) {
        ...
        let mut request: OsURequest = ctx.ireduce_scatter_block(&sendbuf, &mut recvbuf, msg_size);
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Non-blocking Reduce_scatter_block", BenchmarkType::NonBlockingCollective);
    }
