//! OSU MPI Non-blocking Allgather Latency Test (v7.5.2)
//!
//! Measures non-blocking collective allgather latency using UCX tag-matching
//! allgather fallback. Reports Overlap, CPU, Communication, Wait, and Init
//! times in microseconds.
//!
//! Requires at least 2 processes. Repeatedly calls iallgather + wait and reports
//! timing breakdown.
//!
//! # Usage
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_iallgather
//! ```
...
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    for i in 0..(iterations + skip) {
        ...
        let mut request: OsURequest = ctx.iallgather(&sendbuf, &mut recvbuf, msg_size);
        let init_time = init_start.elapsed_us();

        // CPU/compute time: do dummy work while collective is in flight
        let comp_start = Wtime::new();
        // No real dummy compute for iallgather — just measure the overlap
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Non-blocking Allgather", BenchmarkType::NonBlockingCollective);
    }
