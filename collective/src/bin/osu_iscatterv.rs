//! OSU MPI Non-blocking Scatterv Latency Test (v7.5.2)
//!
//! Measures non-blocking collective scatterv latency using UCX tag-matching
//! scatterv fallback. Reports Overlap, CPU, Communication, Wait, and Init
//! times in microseconds.
//!
//! Requires at least 2 processes. Repeatedly calls iscatterv + wait and reports
//! timing breakdown.
//!
//! # Usage
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_iscatterv
//! ```
...
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    // Fixed message size for now (equal counts, root=0)
    let msg_size = 0;
    let sendbuf = vec![0u8; msg_size * size];
    let mut recvbuf = vec![0u8; msg_size];
    ...
    for i in 0..(iterations + skip) {
        ...
        let mut request: OsURequest = ctx.iscatterv(&sendbuf, &mut recvbuf, &counts, &displs, 0);
        let init_time = init_start.elapsed_us();

        // CPU/compute time: do dummy work while collective is in flight
        let comp_start = Wtime::new();
        // No real dummy compute for iscatterv — just measure the overlap
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Non-blocking Scatterv", BenchmarkType::NonBlockingCollective);
    }
