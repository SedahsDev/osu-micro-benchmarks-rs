//! OSU MPI Neighborhood Alltoall Latency Test
//!
//! Measures neighbor alltoall latency using UCX tag-matching fallback.
//! Ring topology: each rank communicates with (rank-1)%size and (rank+1)%size.
//!
//! # Usage
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_neighbor_alltoall
//! ```
...
/// Run the neighbor alltoall latency benchmark.
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    for msg_size in message_sizes(args) {
        ...
        // Warmup iterations
        for _ in 0..skip {
            ctx.neighbor_alltoall(send_slice, recv_slice, msg_size);
            ctx.barrier();
        }
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "Neighbor Alltoall Latency", BenchmarkType::CollectiveLatency);
