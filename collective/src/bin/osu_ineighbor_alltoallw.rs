//! OSU MPI Non-blocking Ineighbor_alltoallw Latency Test (v7.5.2)
//!
//! Measures non-blocking neighbor alltoallw latency using a ring topology.
...
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    loop {
        ...
        for i in 0..(iterations + skip) {
            ...
            let mut request = ctx.ineighbor_alltoallw(
                &sendbuf,
                &mut recvbuf,
                &send_counts,
                &recv_counts,
...
fn main() {
    ...
    if ctx.rank() == 0 {
        ...
        output::print_header(
            &mut out,
            "Non-blocking Ineighbor_alltoallw",
