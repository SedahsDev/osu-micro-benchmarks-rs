//! OSU MPI Alltoallw Latency Test (v7.5.2)
//!
//! Measures alltoallw latency using UCX tag-matching fallback.
//!
//! Requires at least 2 processes. Runs alltoallw with various message sizes,
//! reporting latency in microseconds. Each peer sends/receives `msg_size` bytes.
//! Since we only use bytes, alltoallw is identical to alltoallv in practice.
...
/// Run the alltoallw latency benchmark.
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    for msg_size in message_sizes(args) {
        ...
        // Warmup iterations
        for _ in 0..skip {
            ctx.alltoallw(send_slice, recv_slice, &sendcounts, &sdispls, &recvcounts, &rdispls);
            ctx.barrier();
        }
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "All-to-Allw Personalized Exchange Latency", BenchmarkType::CollectiveLatency);
