//! OSU MPI_Put Bandwidth Test
//!
//! Measures one-sided Put bandwidth using UCX RMA primitives.
//! Rank 0 performs window_size rma_put operations per iteration to Rank 1's
...
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_put_bw
//! ```
...
/// Run the Put bandwidth benchmark.
///
/// Rank 0 performs RMA Puts to Rank 1's registered memory region.
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "MPI_Put Bandwidth", BenchmarkType::Bandwidth);
