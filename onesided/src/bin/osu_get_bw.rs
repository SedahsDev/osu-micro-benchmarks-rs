...
//!
//! Measures one-sided Get bandwidth using UCX RMA primitives.
//! Rank 0 performs window_size rma_get operations per iteration to Rank 1's
//! registered memory region, using flush for fence-like synchronization.
//!
//! Matches C reference behavior:
//!   for j in 0..window_size:
//!       MPI_Get(rbuf + j*size, size, MPI_CHAR, 1, disp + j*size, size, MPI_CHAR, win)
//!   MPI_Win_flush(1, win)
//!
...
/// Run the Get bandwidth benchmark.
///
/// Rank 0 performs RMA Gets from Rank 1's registered memory region.
...
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    for msg_size in message_sizes(args) {
        ...
        // Allocate receive buffer large enough for window_size * msg_size
        let mut recv_buf = vec![0u8; msg_size * window_size];
...
/// Perform window_size Get operations to fill recv_buf.
fn do_get_bw(
    ep: &ucx_sys::ep::Ep,
    recv_buf: &mut [u8],
    msg_size: usize,
    ...
    for j in 0..window_size {
        ...
        let slice = &mut recv_buf[offset..offset + msg_size];
        ep.rma_get(slice, remote_offset, rkey, param)
            .expect("rma_get");
    }
}
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "MPI_Get Bandwidth", BenchmarkType::Bandwidth);
