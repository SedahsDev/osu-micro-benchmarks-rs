//! OSU MPI_Get Latency Test
//!
//! Measures one-sided Get latency using UCX RMA primitives.
...
//!
//! ```bash
//! prterun -np 2 ./target/release/osu_get_latency
//! ```
...
/// Run the Get latency benchmark.
///
/// Rank 0 performs RMA Get from Rank 1's registered memory region.
...
fn run_benchmark(ctx: &OsUContext, args: &CliArgs) {
    ...
    for msg_size in message_sizes(args) {
        ...
        for _ in 0..skip {
            if rank == 0 {
                // Initiator: flush -> get -> flush (fence semantics)
                flush_blocking(worker, &flush_param);
                ep.rma_get(&mut recv_buf, remote_addr, rkey, &rma_param)
                    .expect("rma_get");
                flush_blocking(worker, &flush_param);
            } else {
...
fn main() {
    ...
    if ctx.rank() == 0 {
        let stdout = io::stdout();
        let mut out = stdout.lock();
        output::print_header(&mut out, "MPI_Get Latency", BenchmarkType::Latency);
