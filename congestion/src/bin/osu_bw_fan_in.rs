...
fn fan_init(ctx: &OsUContext) -> FanTopology {
    ...
    if size < 2 {
        eprintln!("[osu_bw_fan_in] Error: Need at least 2 processes");
        process::exit(1);
    }
    ...
    if rank == 0 {
            ...
            if idx > 0 && rank_by_name[idx].0 != rank_by_name[idx - 1].0 {
                if let Some(fp) = first_ppn {
                    if fp != local_rank_counter {
                        eprintln!(
                            "[osu_bw_fan_in] Error: Please run with same ppn for all nodes \
                             (got {} and {})",
        ...
        if total_nodes == 1 {
            eprintln!(
                "[osu_bw_fan_in] Warning: Running on single node — \
